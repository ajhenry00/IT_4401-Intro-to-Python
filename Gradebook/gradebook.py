...
assignment_count = len(gradebook[0])
for column_list in range(assignment_count):
    column = [row[column_list] for row in gradebook]
    average_assignment = statistics.mean(column)
    print("Assignment " + str(column_list + 1) + ": {:.2f}".format(average_assignment))

student_count = len(gradebook)
print("\nStudent Averages:")
for row in range(student_count):
    print("Student " + str(row + 1) + ": {:.2f}".format(statistics.mean(gradebook[row])))
# print(column1)
# print(gradebook)
